...
class TodoTxt(kp.Plugin):

    DEFAULT_ADD_TASK_LABEL = "atodo"
    DEFAULT_DELETE_TASK_LABEL = "dtodo"
    ...
    def on_execute(self, item, action):
        ...
        if action == self.add_task_label:
            num_lines = sum(1 for line in open(self.todo_path))
            f = open(self.todo_path, "a+")
            f.write(str(num_lines + 1) + "." + target + "\n")
            f.close()
